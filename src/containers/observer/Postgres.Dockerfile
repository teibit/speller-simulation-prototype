# Dockerfile for the Postgres observer container.
...
COPY src/containers/observer/src/observer.py .
COPY src/containers/observer/requirements.txt .
...
# source code (observer.py), the requirements file, and the src/common
...
ENTRYPOINT ["python3", "-u", "observer.py", "Postgres"]
